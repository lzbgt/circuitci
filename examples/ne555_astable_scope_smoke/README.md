...
It also includes rising threshold-crossing, minimum high pulse-width, and duty
cycle assertions for `v_out`, display-only KiCad symbol bindings, textbook
orientation metadata, and schematic wire-route waypoints so the Sketch view
opens as a connected, readable circuit. The saved coordinates are generated by
the Sketch classical Auto Layout engine using source-seeded ranks, rails, KiCad
pin anchors, and orthogonal routes; electrical connectivity still comes from
the component pin-to-net bindings and the file-backed SPICE deck.

...
SPICE. The `rc_monte_carlo` sweep samples `R1.value_ohm` with the default
uniform distribution and `C1.value_f` with the `normal` distribution, where the
declared tolerance is treated as +/-3 sigma and clamped to that range. It then
runs the normal AC/Bode observation once per sample.

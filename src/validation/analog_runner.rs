...
use super::analog_embedded_ngspice::{
    EmbeddedCommands, embedded_ngspice_available, run_embedded_ngspice,
};
...
pub(super) fn normalized_frequency_sweep_type(value: Option<&str>) -> Result<&'static str, String> {
    match value.unwrap_or("dec").to_ascii_lowercase().as_str() {
        "dec" => Ok("dec"),
        "oct" => Ok("oct"),
        "lin" => Ok("lin"),
        other => Err(format!(
            "frequency sweep type must be dec, oct, or lin; got {other}."
        )),
    }
}

pub(super) fn normalized_frequency_sweep_type_upper(
    value: Option<&str>,
) -> Result<&'static str, String> {
    Ok(match normalized_frequency_sweep_type(value)? {
        "dec" => "DEC",
        "oct" => "OCT",
        "lin" => "LIN",
        _ => unreachable!("normalized_frequency_sweep_type only returns supported values"),
    })
}
...
pub(super) fn uses_capacitor_initial_conditions(
    bound: &BoundBoard<'_>,
    scenario: &Scenario,
) -> bool {

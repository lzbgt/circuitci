...
use crate::suite::{
    ValidationReportOptions, run_suite, validate_and_write_project_report_with_options,
};
...
impl RepairYamlFinding {
    fn as_repair_kind(self) -> BoardYamlRepairFindingKind {
        match self {
            ...
            Self::ModelNotFound => BoardYamlRepairFindingKind::ModelNotFound,
...
pub fn run() -> Result<()> {
    ...
    match args.command {
        ...
        Some(Command::VerifyModelPackage {
            ...
            registry_entry,
            output,
        }) => model_package::run_verify_model_package(lock, registry, registry_entry, output),
        Some(Command::VerifyModelPackageBundle { bundle, output }) => {
            model_package::run_verify_model_package_bundle(bundle, output)
        }
        Some(Command::InstallModelPackageBundle {
            ...
            output,
        }) => model_package::run_install_model_package_bundle(
            bundle,
            install_dir,
            registry_output,
            registry_entry,
        ...
        Some(Command::ImportModelPackageBundle {
            ...
            max_runtime_ms,
            output,
        }) => model_package::run_import_model_package_bundle(
        ...
        Some(Command::ExportModelPackage {
            ...
            registry_entry,
            registry_artifact_id,
        }) => model_package::run_export_model_package(model_package::CliModelPackageExportArgs {
            package_name,
            package_version,
            package_artifacts,
            artifact_id,
        ...
        Some(Command::ExportModelConformanceReport {
            ...
            solver,
            output,
        }) => model_package::run_export_model_conformance_report(
            report,
            package_name,
            package_version,
            artifact_id,
        ...
        Some(Command::ExportModelPackageBundle {
            ...
            model_package::run_export_model_package_bundle(lock, registry, registry_entry, output)
        }
        Some(Command::MergeModelPackageRegistry {
            base,
            inputs,
            output,
        }) => model_package::run_merge_model_package_registry(base, inputs, output),
...
fn run_validate(
    ...
    let bundle_imports = model_package_bundle_imports
        .iter()
        .map(|spec| model_package::parse_validation_bundle_import_spec(spec))

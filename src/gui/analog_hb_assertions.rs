...
#[derive(Debug, Clone)]
pub(super) struct AnalogHarmonicBalanceAssertionDraft {
    pub(super) scenario_name: String,
    pub(super) assertion_name: String,
    pub(super) harmonic: u32,
    pub(super) metric: String,
    pub(super) relation: String,
    pub(super) threshold: f64,
}

pub(super) fn append_analog_hb_assertion(
    text: &str,
    draft: &AnalogHarmonicBalanceAssertionDraft,
) -> Result<String> {
    validate_hb_assertion_draft(draft)?;
    ...
    if scenario.scenario_type != "analog_harmonic_balance" {
        anyhow::bail!("HB assertions require an analog_harmonic_balance scenario.");
    }
    let analog = scenario
        .analog
        .as_ref()
    ...
    {
        anyhow::bail!(
            "HB assertion {} already exists in scenario {}.",
    ...
    let assertions = ensure_child_sequence_mut(
        analysis_mapping,
        "hb_assertions",
        "harmonic-balance assertions",
    )?;
    assertions.push(hb_assertion_value(draft)?);
    let updated =
    ...
    let _: crate::board_ir::BoardProject = serde_yaml_ng::from_str(&updated)
        .context("Edited HB assertion YAML is not valid Board IR.")?;
    Ok(updated)
}

pub(super) fn unique_analog_hb_assertion_name(
    text: &str,
    scenario_name: &str,
    requested_name: &str,
) -> Result<String> {
...
fn validate_hb_assertion_draft(draft: &AnalogHarmonicBalanceAssertionDraft) -> Result<()> {
    validated_id(&draft.scenario_name, "scenario name")?;
    validated_id(&draft.assertion_name, "assertion name")?;
    if draft.harmonic > 1024 {
        anyhow::bail!("HB assertion harmonic must be between 0 and 1024.");
    }
    if !matches!(
        draft.metric.as_str(),
        "magnitude" | "phase_deg" | "real" | "imaginary"
    ) {
        anyhow::bail!("Unsupported HB assertion metric {}.", draft.metric);
    }
    if !matches!(draft.relation.as_str(), "above" | "below") {
        anyhow::bail!("HB assertion relation must be above or below.");
    }
    if !draft.threshold.is_finite() {
        anyhow::bail!("HB assertion threshold must be finite.");
    }
    Ok(())
}

fn hb_assertion_value(draft: &AnalogHarmonicBalanceAssertionDraft) -> Result<serde_yaml_ng::Value> {
    let mut assertion = serde_yaml_ng::Mapping::new();
    insert_string(&mut assertion, "name", draft.assertion_name.trim());
    assertion.insert(
        key("harmonic"),
        serde_yaml_ng::to_value(draft.harmonic).context("Failed to encode HB harmonic.")?,
    );
    insert_string(&mut assertion, "metric", draft.metric.trim());
    insert_string(&mut assertion, "relation", draft.relation.trim());
    assertion.insert(
        key("threshold"),
        serde_yaml_ng::to_value(draft.threshold).context("Failed to encode HB threshold.")?,
    );
...
mod tests {
    ...
    #[test]
    fn append_hb_assertion_emits_valid_analysis_yaml() {
        let edited = append_analog_hb_assertion(hb_project_yaml(), &hb_assertion_draft()).unwrap();
        ...
        assert_eq!(assertions[0].name, "h1_magnitude_floor");
        assert_eq!(assertions[0].harmonic, 1);
        assert_eq!(
            assertions[0].metric,
            crate::board_ir::AnalogHarmonicBalanceMetric::Magnitude
        );
        assert_eq!(
            assertions[0].relation,
            crate::board_ir::AnalogRelation::Above
        );
        assert_eq!(assertions[0].threshold, 0.2);
    }

    #[test]
    fn unique_hb_assertion_name_suffixes_collisions() {
        let edited = append_analog_hb_assertion(hb_project_yaml(), &hb_assertion_draft()).unwrap();
        let name =
            unique_analog_hb_assertion_name(&edited, "hb_out", "h1_magnitude_floor").unwrap();

        assert_eq!(name, "h1_magnitude_floor_2");
    }

    #[test]
    fn append_hb_assertion_rejects_wrong_scenario_type() {
        let error =
            append_analog_hb_assertion(non_hb_project_yaml(), &hb_assertion_draft()).unwrap_err();

        assert!(
            error
                .to_string()
                .contains("analog_harmonic_balance scenario")
        );
    }

    fn hb_assertion_draft() -> AnalogHarmonicBalanceAssertionDraft {
        AnalogHarmonicBalanceAssertionDraft {
            scenario_name: "hb_out".to_string(),
            assertion_name: "h1_magnitude_floor".to_string(),
            harmonic: 1,
            metric: "magnitude".to_string(),
            relation: "above".to_string(),
            threshold: 0.2,
        }
    }

    fn hb_project_yaml() -> &'static str {
        r#"
project:
  name: hb_gui_test
  version: 0.1.0
...
scenarios:
  - name: hb_out
    ...
    analog:
      ...
      analysis:
        type: hb
        hb_fundamental_frequency_hz: 1000000
        hb_harmonics: 3
...
"#
    ...
    fn non_hb_project_yaml() -> &'static str {
        r#"
project:
  name: hb_gui_test
  version: 0.1.0

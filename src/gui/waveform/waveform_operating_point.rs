        ...
        .show(ui, |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                if ui.button("Copy CSV").clicked() {
                    ui.ctx()
                        .copy_text(operating_point_views_csv(&self.operating_points));
...
pub(super) fn operating_point_views_csv(views: &[OperatingPointView]) -> String {
    let mut csv = String::from("scenario,sweep,corner,probe,value,artifact\n");
    for view in views {
        let sweep = view.sweep.as_deref().unwrap_or("nominal");
        let corner = view.corner.as_deref().unwrap_or("nominal");
        for value in &view.values {
...
mod tests {
    ...
    fn operating_point_csv_export_flattens_corner_values() {
        ...
        .unwrap();
        let csv = operating_point_views_csv(&[view]);
        assert!(csv.starts_with("scenario,sweep,corner,probe,value,artifact\n"));
        assert!(csv.contains("divider_dc_bias,divider_tolerance,corner_001,vin,5.000000000000e0"));
        assert!(
            csv.contains("divider_dc_bias,divider_tolerance,corner_001,midpoint,2.500000000000e0")
        );
    }

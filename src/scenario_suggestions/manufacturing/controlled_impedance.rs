...
use crate::board_ir::{
    ...
    ControlledImpedanceDifferentialPairTarget, ControlledImpedanceNetTarget, LayoutCopper, NetKind,
...
pub(super) fn controlled_impedance_suggestions(
    ...
    suggestions.extend(
        super::controlled_impedance_solver::controlled_impedance_solver_result_suggestions(
            bound,
            project_name,
        ),
    );
...
pub(super) fn unordered_pair_matches(
    first: &str,
    second: &str,
    expected_first: &str,
    expected_second: &str,
...
pub(super) fn non_negative_finite(value: f64) -> bool {
    value.is_finite() && value >= 0.0
}
...
pub(super) fn routes_have_parallel_gap_evidence(
...
pub(super) fn usable_route_segment(segment: &RouteSegment) -> bool {

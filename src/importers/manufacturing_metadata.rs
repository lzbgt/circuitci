...
pub fn import_manufacturing_metadata(
    ...
    let manifest = ImportManifest {
        schema_version: "0.41.0".to_string(),
        sources: SourceManifest {
...
fn apply_metadata(
    ...
    for field in fields {
        ...
        if field.field == ManufacturingField::ControlledImpedanceNet {
            ...
            wrote_manufacturing = true;
        } else if field.field == ManufacturingField::ControlledImpedanceSolverMaterialProcess {
            let process = field
                .controlled_impedance_solver_material_process
                .as_ref()
                .context(
                    "controlled_impedance_solver_material_process field must have process value",
                )?;
            let value = normalized_yaml_value(field)?;
            let manufacturing = ensure_mapping_field_mut(board, "manufacturing")?;
            let controlled_impedance =
                ensure_mapping_field_mut(manufacturing, "controlled_impedance")?;
            let processes =
                ensure_sequence_field_mut(controlled_impedance, "solver_material_processes")?;
            upsert_named_sequence_value(
                processes,
                &process.name,
                value,
            ...
            wrote_manufacturing = true;
        } else if field.field == ManufacturingField::ControlledImpedanceSolverRuntimeAllowlist {
            let allowlist = field
                .controlled_impedance_solver_runtime_allowlist
                .as_ref()
                .context(
                    "controlled_impedance_solver_runtime_allowlist field must have allowlist value",
                )?;
            let value = normalized_yaml_value(field)?;
            let manufacturing = ensure_mapping_field_mut(board, "manufacturing")?;
            let controlled_impedance =
                ensure_mapping_field_mut(manufacturing, "controlled_impedance")?;
            let allowlists =
                ensure_sequence_field_mut(controlled_impedance, "solver_runtime_allowlists")?;
            upsert_named_sequence_value(
                allowlists,
                &allowlist.name,
                value,
            ...
            wrote_manufacturing = true;
        } else if field.field == ManufacturingField::ControlledImpedanceSolverExecutionEnvironment {
            let environment = field
                .controlled_impedance_solver_execution_environment
                .as_ref()
                .context(
                    "controlled_impedance_solver_execution_environment field must have environment value",
                )?;
            let value = normalized_yaml_value(field)?;
            let manufacturing = ensure_mapping_field_mut(board, "manufacturing")?;
            let controlled_impedance =
                ensure_mapping_field_mut(manufacturing, "controlled_impedance")?;
            let environments =
                ensure_sequence_field_mut(controlled_impedance, "solver_execution_environments")?;
            upsert_named_sequence_value(
                environments,
                &environment.name,
                value,
            ...
            wrote_manufacturing = true;
        } else if field.field == ManufacturingField::ControlledImpedanceSolverRunLog {
            let run_log = field
                .controlled_impedance_solver_run_log
                .as_ref()
                .context("controlled_impedance_solver_run_log field must have run-log value")?;
            let value = normalized_yaml_value(field)?;
            let manufacturing = ensure_mapping_field_mut(board, "manufacturing")?;
            let controlled_impedance =
                ensure_mapping_field_mut(manufacturing, "controlled_impedance")?;

...
use types::{
    AnalogScenarioMapping, AnalogYaml, AssertionAggregationYaml, BoardLayoutYaml, BoardYaml,
    ComponentLayoutMapping, ComponentMapping, ComponentSourceInstanceYaml, ComponentSourceYaml,
    ComponentSpiceYaml, ComponentYaml, EndpointYaml, GeneratedNetlistYaml, ImportedComponentModel,
    KicadMapping, LayoutEntryApertureYaml, LayoutEntryClearanceYaml, LayoutEntryDirectionYaml,
    LayoutFootprintYaml, MappedNetKind, ModelFileYaml, NetMapping, NetYaml, NodeBindingYaml,
    PinBindingYaml, ProjectMetaYaml, ProjectYaml, ScenarioYaml, SpicePrimitiveYaml,
    SpiceValueSourceYaml,
};
use walkdir::WalkDir;
...
pub fn import_kicad_netlist(options: &KicadImportOptions) -> Result<()> {
    import_kicad_netlist_with_progress(options, |_, _| {})
}

pub fn import_kicad_netlist_with_progress<F>(
...
{
    import_kicad_netlist_with_progress_and_cancel(options, &mut on_progress, || false)
}

pub fn import_kicad_netlist_with_progress_and_cancel<F, C>(
    options: &KicadImportOptions,
    mut on_progress: F,
    should_cancel: C,
) -> Result<()>
...
pub(super) fn import_parsed_kicad_with_progress_and_cancel<F, C>(
    options: &KicadImportOptions,
    ...
    C: Fn() -> bool,
{
    on_progress(
        ...
        options
            .mapping
            .as_ref()
            .map(|path| format!("Reading {}.", path.display()))
            .unwrap_or_else(|| "Using default import mapping.".to_string()),
    );
...
fn validate_probe_assertion_contract(scenario: &AnalogScenarioMapping) -> Result<()> {
    ...
    for assertion in &scenario.assertions {
        ...
        let is_count = matches!(
            assertion.aggregation,
            Some(
                AssertionAggregationYaml::CrossingCount
                    | AssertionAggregationYaml::RisingCrossingCount
                    | AssertionAggregationYaml::FallingCrossingCount
            )
        );
        if is_window {

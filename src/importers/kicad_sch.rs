use super::kicad::{
    ...
    ParsedNode, import_parsed_kicad_with_progress_and_cancel,
};
pub(crate) mod sexp;
mod symbols;
...
pub fn import_kicad_schematic(options: &KicadImportOptions) -> Result<()> {
    import_kicad_schematic_with_progress(options, |_, _| {})
}

pub fn import_kicad_schematic_with_progress<F>(
...
{
    import_kicad_schematic_with_progress_and_cancel(options, &mut on_progress, || false)
}

pub fn import_kicad_schematic_with_progress_and_cancel<F, C>(
    options: &KicadImportOptions,
    mut on_progress: F,
    should_cancel: C,
) -> Result<()>
...
fn ensure_not_canceled(should_cancel: &impl Fn() -> bool) -> Result<()> {
    if should_cancel() {
        bail!("KiCad schematic import canceled before completion.");
    }
    Ok(())
}
...
mod tests {
    use super::{
        import_kicad_schematic_with_progress, import_kicad_schematic_with_progress_and_cancel,
        parse_kicad_schematic,
    };
    use crate::importers::kicad::KicadImportOptions;
...
"#,
    ...
    #[test]
    fn import_kicad_schematic_with_progress_emits_phases() {
        let dir = tempfile::tempdir().unwrap();
        let schematic = dir.path().join("basic.kicad_sch");
...
"#,
        ...
        import_kicad_schematic_with_progress(
            &KicadImportOptions {
                input: schematic,
                output,
                name: "progress_kicad".to_string(),
                default_model: "generic.schematic.imported_component".to_string(),
                mapping: None,
            },
    ...
    #[test]
    fn import_kicad_schematic_cancellation_stops_before_write() {
        let dir = tempfile::tempdir().unwrap();
        let schematic = dir.path().join("basic.kicad_sch");
...
"#,
        ...
        let error = import_kicad_schematic_with_progress_and_cancel(
            &KicadImportOptions {
                input: schematic,
                output: output.clone(),
                name: "canceled_kicad".to_string(),
                default_model: "generic.schematic.imported_component".to_string(),
                mapping: None,
            },
            |_, _| {},

...
behavior, including loaded-artifact unload actions that drop or shift transient trace references. `src/gui/waveform/waveform_footprint.rs` owns loaded-waveform footprint readouts, diagnostics-derived source classification/grouping, sort/filter projection, visible-row CSV copy/export, memory-budget warnings, and guarded visible-row or largest-first bulk unload preview/confirmation.

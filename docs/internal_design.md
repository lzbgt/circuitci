...
cancel-aware waveform CSV parsing, transient waveform-load diagnostics, Scopes state orchestration, simulation-time scrub/playback
...
so large CSV parsing does not run on the UI thread, skipped traces are visible
to the user, and stale waveform data does not outlive its report.

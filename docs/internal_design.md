...
low-pass filter for comparing input/output traces and frequency readouts. The
same registry now includes SPICE-backed op-amp/comparator, regulator,
load-switch, charger, power-path, reset, Bode, DC, noise, and Monte Carlo
fixtures. Direct-open fixtures carry schematic-only `wire_routes` waypoints so
example loading exercises readable connected Sketch layouts without changing
solver connectivity.
`src/gui/project.rs`
...
`src/gui/simulation_probe_assertions.rs` owns the selected-probe assertion
table plus canvas-triggered assertion add, clear, edit, delete, and
quick-threshold actions for probe elements.
...
drivers, PWM drivers, IMUs, environmental sensors, SPI flash, MCU line-state
checks, level shifters, logic buffers, and protection clamps, keeping the
orchestration module below the 2000-line source guard as more model families
are added.
...
candidate edits in `status: proposed` for cheap agent inspection. Report-driven

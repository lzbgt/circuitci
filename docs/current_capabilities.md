...
| --- | --- | --- |
| CLI | Default `circuitci` binary for import, suggestion, validation, suites, report generation, reviewed manufacturing metadata CSV import, and the first Board IR YAML repair loop via `repair-yaml` for `INVALID_POWER_DOMAIN`, `NET_NOT_FOUND`, `PIN_NOT_DECLARED`, and `REQUIRED_PIN_FLOATING` findings plus additive `analog-model-package-metadata` migration. | Primary automation surface for CI and agents. YAML repair is intentionally narrow: it edits only copied projects, changes an existing non-power net to `kind: power` when a model power pin proves that classification, adds a missing net with kind inferred from declared model port kind, removes a stray pin binding not declared by the resolved model, connects a missing required pin only to an existing compatible net already named by component power-domain metadata, or adds missing package-lock/registry metadata to generated analog model files when the component library provides matching provenance. `repair-yaml --dry-run` writes original validation plus candidate proposals without writing or validating a repaired copy; `--apply-report` replays a matching dry-run report, and repeated `--proposal-id` values select a subset of proposed edits for partial apply. Repair reports include stable `reason_codes[]` and proposal-level `reason_code` values so agents can branch on blocked, skipped, no-op, and remaining-finding cases without parsing prose. |

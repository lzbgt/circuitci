...
| SPICE | `import-spice` preserves deck nodes, model includes, probes, a file-backed transient scenario, and ngspice raw `.meas`/`meas` cards as a sibling `analog_measure` scenario. | Imported decks need assertions for sign-off and accurate external models; raw measure cards remain ngspice-specific and explicit Xyce runs fail closed. |
...
| JLC/EasyEDA assembly | BOM/CPL import with JSON source/row-hash/component/footprint-key/side-confidence/orientation-confidence manifest, EasyEDA Pro envelope inspection with Markdown plus JSON table metadata/content-hash, payload-hash, object-hash, and plaintext-shape manifest, flying-probe pad/net evidence. | Assembly evidence does not imply schematic intent, electrical pin behavior, package compatibility, or final assembly polarity by itself; opaque EasyEDA Pro history payloads block geometry conversion. |
...
`vendor.winbond.w25q64jv` provides a source-backed model for Winbond's
...
generated-SPICE standby observation face for VCC and SPI/QSPI idle-bias lines.
It does not emulate SPI commands, JEDEC ID, SFDP tables, flash contents, XIP
...
low-power current metadata, I2C/SPI clock-class metadata, SDO-selected I2C
address metadata, and a reduced high-impedance generated-SPICE I2C observation
face for rails, pull-ups, `CSB`, and `SDO`. It does not validate sensor
accuracy, compensation algorithms, register protocol, bus timing, response

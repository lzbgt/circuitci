...
| Model quality | `LOW_CONFIDENCE_MODEL` report limitations, blocking `MODEL_QUALITY_REQUIRED` sign-off gates, fail-closed OpenVAF/OSDI model compiler provenance checks for compiled Verilog-A compact-model artifacts, report/solver-manifest `model_file_provenance[]` records that distinguish prebuilt from CI-rebuilt OSDI artifacts, external-ngspice `pre_osdi` wrapper loading, explicit Xyce/embedded backend rejection for OSDI artifacts, and opt-in real ngspice/OpenVAF OSDI conformance through `CIRCUITCI_RUN_REAL_NGSPICE_OSDI=1 cargo test --test analog_model_compiler_cli`. |
...
- Successful analog solver runs write a versioned `solver_manifest.json`
  ...
  OpenVAF/OSDI path is external-ngspice-only; Xyce compact-model loading needs
  a separate Xyce/ADMS plugin contract, not the same `*.osdi` artifacts.

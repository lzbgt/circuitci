...
| --- | --- | --- |
| CLI | Default `circuitci` binary for import, suggestion, validation, suites, report generation, and the first Board IR YAML repair loop via `repair-yaml` for `INVALID_POWER_DOMAIN`, `NET_NOT_FOUND`, and `PIN_NOT_DECLARED` findings. | Primary automation surface for CI and agents. YAML repair is intentionally narrow: it edits only copied projects, changes an existing non-power net to `kind: power` when a model power pin proves that classification, adds a missing net with kind inferred from declared model port kind, or removes a stray pin binding not declared by the resolved model. |

...
| Manufacturing | Drill diameter, drill/slot edge clearance, slot width/aspect, annular ring, copper edge/spacing, explicit same-layer conductor creepage/clearance screens, controlled-impedance route geometry checks against reviewed target evidence, controlled-impedance stackup material/copper-thickness evidence checks, stackup-aware adjacent-plane return-path coverage, slot-crossing, stitching-via transition screens, reviewed RF antenna keepout-to-copper and feed-path route/proximity screens, reviewed thermal copper-area plus thermal via/stackup evidence screens, solder-mask opening/dam, solder-paste opening/size/area/spacing, IC/BGA paste aperture screens, assembly-vs-footprint evidence alignment, explicit pin-1 marker orientation, plus reviewed order/process CSV import for `board.manufacturing` metadata with a JSON row-evidence manifest. |

...
6. Evaluate AC/Bode assertions over `bode.csv`, including gain and phase at a
   ...
   margin at the falling 0 dB gain crossing, and gain margin at the falling
   -180 degree phase crossing. Failed assertions emit critical
   `SPICE_AC_ANALYSIS` findings with measured and limit data.
7. Preserve `bode.csv` paths in the report `waveforms` list so the GUI Scopes
   view can load them as frequency-axis traces with magnitude/phase lanes,
   derived group-delay traces, and sweep-corner comparison support.
...
8. Optional `analysis.s_parameter_noise_assertions[]` signs off two-port RF
   ...
   `optimum_source_reflection_magnitude_max`. When RF noise assertions are
   present, `backend: ngspice` emits RF port voltage sources, runs
   `.SP ... donoise=1`, retains `s_parameters_raw.csv` and
   `s_parameter_noise_raw.csv`, normalizes the S matrix into
   `s_parameters.csv`, normalizes worst-case NF, NFmin, Rn, and `|SOpt|` into
   `s_parameter_noise_summary.csv`, records both normalized outputs in
...
5. Optional `analysis.transfer_function_assertions[]` sign off
   `transfer_function_summary.csv` metrics (`transfer_function_gain`,
   `input_resistance_ohm`, or `output_resistance_ohm`) with `above`/`below`
   limits. Missing/malformed summaries and failed limits emit
   `SPICE_TRANSFER_FUNCTION_ANALYSIS` findings with retained summary
   provenance.
6. Validation reports project retained `transfer_function_summary.csv` rows
   into top-level `transfer_function_summaries[]`, Markdown reports include a
   "Transfer Function Summary" section, and GUI Scopes surfaces compact `.TF`
   scalar rows in the report artifact panel.
7. Opt-in real-ngspice
...
7. Optional `analysis.distortion_assertions[]` entries sign off normalized
   component maxima from `distortion_summary.csv`. Each assertion names the
   component label, uses `relation: below|above`, and compares the component's
   `max_magnitude` against a non-negative threshold with optional unit and
   suggested fixes. Missing components fail closed because the assertion is
   bound to the normalized solver artifact, not to backend-specific raw text.
8. Validation reports project `distortion_summary.csv` rows into top-level

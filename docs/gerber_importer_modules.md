...
- `net` and zone-derived `island_id` from zone containment in
  `board.layout.zones`,
- solder-mask flash/draw/region opening owner metadata from unique pad or via
  overlap on the corresponding copper layer,
- solder-paste flash/draw/region opening owner metadata from unique pad overlap
  on the corresponding copper layer.

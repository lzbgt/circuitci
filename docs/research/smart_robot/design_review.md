...
- `U_SERVO_SW` selected as a configured TI TPS25948 8 A eFuse for first-pass
...
downstream rail capacitance from CAD/BOM before fabrication sign-off.
...
- CSD88599Q5DC SOA validation is now explicit:
  `MOTOR_BRIDGE_SOA_VALID` consumes the TI Figure 4-3 typical system SOA curve
  digitized as output current versus board temperature. The current wheel
...
layout evidence, cable assembly current/temperature-rise/voltage-drop

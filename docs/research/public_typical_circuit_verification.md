...
| Aosong AHT20 humidity/temperature sensor power and observation use | <https://www.aosong.com/userfiles/files/media/Data%20Sheet%20AHT20.pdf> | `docs/research/datasheets/aosong/aht20_datasheet_v1_0_2021.pdf` |
...
`suites/public_typical_circuits.yaml` combines 142 public-reference passing
cases and 172 paired injected-error
cases:
...
| `aosong_aht20_i2c_power_passes` | `examples/good_aosong_aht20_i2c_power/project.yaml` | pass | AHT20 humidity/temperature sensor on a 3.3 V `VDD` rail with VDD-referenced idle SDA/SCL pull-ups. |
| `aosong_aht20_i2c_observation_passes` | `examples/good_aosong_aht20_i2c_observation/project.yaml` | pass | AHT20 generated-SPICE observation whose model file is resolved and SHA-pinned from `simulation.spice.model_path`, with source-backed 3.3 V `VDD` and idle-high `SDA`/`SCL`. |
| `aosong_aht20_vdd_overvoltage_detected` | `examples/bad_aosong_aht20_vdd_overvoltage/project.yaml` | fail | Detects AHT20 `VDD` connected to a 6 V rail above the source-backed 5.5 V maximum. |
...
```text
CircuitCI suite public_typical_circuits: pass (cases=314, passed=314, failed=0)
```
...
| `aosong_aht20_vdd_overvoltage_detected` | `POWER_TREE_VALID` | Power rail `rail_6v` supplies `UAHT.VDD` at `6.000000 V`, outside the model maximum operating voltage `5.500000 V`. |
...
All 142 public-reference pass cases produced zero critical
findings.
All 172 paired injected-error cases failed with the expected
critical finding ID, and all 172 repair-pair checks passed.

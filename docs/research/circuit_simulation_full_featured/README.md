...
- The first `.TF` path is external-ngspice execution with normalized
  ...
  these assertions and hydrates failed `.TF` findings back into the editor, and
  the GUI Run Setup editor now authors generated-from-board
  `analog_transfer_function` scenarios from a selected output probe net and
  input source.
...
- 2026-07-04: Extended `backend: auto` Xyce fallback to `.SENS` and `.FOUR`.
  The explicit Xyce adapters already emit the same normalized
  `sensitivity_summary.csv` and `fourier_summary.csv` contracts as ngspice,
...
- The first DC sweep path is an external-ngspice adapter with a Board
  ...
  implemented. The GUI Run Setup editor now authors generated-from-board
  `analog_dc_sweep` scenarios, and the GUI DC Sweep Check editor authors and
  hydrates failed `dc_sweep_assertions[]`, backed by retained failure metadata
  for the probe, aggregation, nearest measured sweep value, and declared sample
  sweep value.
...
- Added the first ngspice-backed small-signal distortion adapter for
  ...
  `distortion_assertions[]` entries. The GUI Run Setup editor now authors
  generated-from-board `analog_distortion` scenarios with
  harmonic/intermodulation mode, selected output probe net, frequency sweep
  bounds, F1 source, and F2 source plus F2/F1 ratio when intermodulation mode is
  selected.

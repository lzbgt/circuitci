...
- `USB_CONNECTOR_ENTRY_CLEARANCE_VALID` checks a static 2D cable-entry corridor
  ...
  explicit scenario `entry_direction_deg`. Optional component-model aperture
  offsets and width can move and widen that 2D corridor, but they still do not
  model plug geometry, connector shell volume, cable bend radius, panel
  cutouts, enclosure interference, or assembly stack-up.

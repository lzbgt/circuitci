...
| `gui` | Optional `egui`/`eframe` desktop shell for project loading, staged review, validation, simulation artifact observation, and report viewing. `gui::sketch` owns Board IR graph snapshots/layout, structured scalar component/net edits, and conservative component/unreferenced-net add/remove mutations. |

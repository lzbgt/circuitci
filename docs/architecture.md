...
| `gui` | Optional `egui`/`eframe` desktop shell for project loading, staged review, validation, simulation artifact observation, and report viewing. `gui::sketch` owns Board IR graph snapshots/layout, structured scalar component/net edits, conservative component/unreferenced-net add/remove mutations, and validated component pin assignment. `gui::simulation` owns the Simulation stage UI, waveform CSV parsing, plotting, cursor readouts, and min/max/delta measurements. `gui::analog` owns generated-from-Board analog transient scenario creation and sample/min/max assertion authoring. |

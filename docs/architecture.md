...
| `suite` | Run acceptance/public fixture suites against a built CLI and provide the shared single-project validation/report helper, including optional progress callbacks used by the GUI. |
| `gui` | Optional `egui`/`eframe` desktop shell for project loading, KiCad/SPICE import, staged review, validation, simulation artifact observation, and report viewing. `gui::shell` owns menu chrome, workflow stage routing, project/status panels, Project and Reports stage views, and finding/limitation rendering. `gui::jobs` owns background validation, scenario suggestion, and KiCad/SPICE import worker launch, progress-event handling, channel polling, stale-result rejection, cancel-request handling, and capped recent-job history records for status-panel diagnostics. `gui::file_dialogs` owns native open/save/folder dialog helpers for project, import, and output path fields. `gui::import_flow` owns Import stage UI and KiCad schematic, KiCad PCB, and SPICE deck import command wiring. `gui::project` owns project YAML load/save/parse actions, import path/name helpers, the shared Board IR YAML undo/redo history used by graph, property, wire, and text edits, and the pending-action dirty-state guard used before load/import/quit replaces unsaved Board IR YAML or file-backed SPICE deck edits. `gui::sketch` owns Board IR graph snapshots/layout, view-state pan/zoom transforms, grid/snap helpers, fit-content bounds, rendered component pin anchors, model-port default pin/net seeding, persisted schematic node positions, visual orthogonal pin-to-pin and pin-to-net assignment, net label/junction rendering, wire hit-testing, component/net/wire/probe-badge context menu routing, and graph hover/runtime tint display. `gui::sketch_probes` owns derived analog voltage/current/power probe badge targeting, layout, drawing, badge hit-testing, and latest-report assertion status derivation. `gui::sketch_inspector` owns selected-item inspector UI, scalar component/net edits, component/net add/remove actions, symbol style edits, pin assignment, visual wire mutations, and canvas-driven voltage/current/power probe insertion/removal controls. `gui::sketch_symbols` owns model/id-inferred common-class symbol selection and egui glyph drawing for sketch nodes. `gui::sketch_actions` owns canvas selection state, fit-content application, group drag/nudge/alignment, and selected-item deletion composition. `gui::library` owns active model browsing, filtering, selected-component model assignment, and model-backed component insertion. `gui::simulation` owns the Simulation stage UI, selected-probe assertion table and row-level edit/delete actions, waveform CSV parsing, plotting, simulation-time scrub/playback, cursor readouts, min/max/delta measurements, runtime probe values for graph hovers, normalized runtime activity for graph tinting, exact probe-value lookup for badge quick assertions, and canvas-triggered assertion add/clear/quick-threshold actions for probe badges. `gui::analog` owns generated-from-Board analog transient scenario creation, selected-net voltage-probe insertion, selected-component source/passive/semiconductor current-probe insertion, selected-component source/passive/semiconductor power-probe insertion into existing analog scenarios, probe assertion summaries, probe removal with dependent assertion cleanup, assertion removal/replacement, assertion removal for a probe, unique assertion naming, and sample/min/max assertion authoring. `gui::spice` owns file-backed SPICE deck discovery, load/save, and save-and-run actions for analog scenarios. |

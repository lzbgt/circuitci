...
| `validation` | Dispatch scenario checks and collect deterministic findings. Analog waveform assertion contracts/report findings live separately from pure waveform measurement math so timing, pulse, energy, settling, overshoot, phase-delay, and setup/hold algorithms remain reusable across validation and tests. |
...
| `gui` | Optional `egui`/`eframe` desktop shell for project loading, KiCad/SPICE import, staged review, validation, simulation artifact observation, and report viewing. Focused core GUI regressions live in `gui::gui_core_tests`, with direct-open example workflow regressions in `gui::project_example_workflow_tests`. `gui::shell` owns menu chrome, workflow stage routing, project/status panels, Project and Reports stage views, and finding/limitation rendering. The Sketch stage follows a schematic-first editor convention: the central canvas is the primary model view, project/library/import details are secondary, and run/scope controls stay directly reachable from the schematic. `gui::jobs` owns background validation, waveform artifact loading, scenario suggestion, and KiCad/SPICE import worker launch, progress-event handling, channel polling, stale-result rejection, cancel-request handling, typed cancellation outcome recording, and capped recent-job history records for status-panel diagnostics. `gui::file_dialogs` owns native open/save/folder dialog helpers for project, import, output, and analog model/include path fields. `gui::import_flow` owns Import stage UI and KiCad schematic, KiCad PCB, SPICE deck import command wiring, and the scope-ready NE555 astable SPICE preset for Run + Scopes workflows. `gui::project` owns project YAML load/save/parse actions, import path/name helpers, the shared Board IR YAML undo/redo history used by graph, property, wire, and text edits, and the pending-action dirty-state guard used before load/import/quit replaces unsaved Board IR YAML or file-backed SPICE deck edits. `gui::project_examples` owns the scope-ready GUI example catalog, expected trace/frequency metadata, and Project/Sketch workflow labels/actions. `gui::sketch` owns Board IR graph snapshots, sketch data types, model-port default pin/net seeding, persisted schematic node positions/styles, schematic wire route waypoints, and shared sketch YAML helpers. `gui::sketch_layout` owns graph layout, role-aware classical fallback placement for items without persisted schematic coordinates, view-state pan/zoom transforms, grid/snap helpers, fit-all and fit-selection bounds, bounded full-list logical layout for pannable imported designs, orthogonal wire geometry, and wire hit-testing. `gui::sketch_layout_pins` owns rendered component pin anchors with net-kind display metadata, KiCad symbol pin geometry projection, high-pin label lane spreading, and component/net node sizing. `gui::sketch_routes` owns shared orthogonal route geometry for display, hit-testing, insertion, active wire preview, and route drag preview. `gui::sketch_wire_draft` owns transient in-progress wire-bend points while a pin connection is being drawn. `gui::sketch_canvas` owns Sketch-stage canvas drawing order, hit-test/event routing, pin-anchor drag-to-wire completion, overview-minimap event routing, and graph hover/runtime routing. `gui::sketch_scope_tools` owns armed V/I/P scope-probe placement tool state, canvas-hover V/I/P shortcuts, shortcut-safe target validation, and same-key/Esc cancellation for canvas click-to-scope workflows. `gui::sketch_scope_feedback` owns the scope-tool hover target projection, valid/invalid feedback geometry, and canvas feedback painting. `gui::scope_auto_probes` owns the Sketch/Scopes Auto Probes action, guarded Auto-before-Run preference, and Run Readiness probe preview for bounded missing voltage probes from analog node bindings and source-branch current probes from supported primitive sources while skipping existing expressions. `gui::sketch_canvas_tools` owns Simulink/Proteus-style viewport input for blank-canvas drag panning, touchpad panning, and pointer-focused zoom, active multi-bend wire drawing, direct wire-route drag editing and clearing, canvas `R` / `Shift+R` rotation, `F` flip, and `Shift+F` pin-side dispatch for selected components or armed component placement, placement orientation controls, canvas probe defaults, and selected-component orientation transforms. `gui::sketch_alignment` owns transient alignment-guide derivation, drawing, and optional guide-snap target adjustment for component placement plus selected-node and selected-group drag affordances while still routing persistence through validated schematic position edits. `gui::sketch_render` owns graph node and pin-anchor painting, including runtime tinting, transient loaded-waveform `scope` chips with shared paint/hit-test geometry, opacity handling, symbol glyph dispatch, and kind-aware pin chips. `gui::sketch_canvas_render` owns canvas-local tooltip and paint helpers for wires, route handles, wire previews, wire target affordances, snap/free target feedback, and orientation-aware placement ghosts with pin-side affordances. `gui::sketch_minimap` owns transient schematic overview drawing and click/drag-to-pan mapping over the current graph bounds. `gui::sketch_canvas_menus` owns component/net/wire/probe-badge/blank-canvas context menu routing, including route-handle actions, named-net/off-page label placement, and runtime Scopes jumps plus direct runtime `scope` chip clicks for nodes that match loaded waveform probes. `gui::sketch_connectivity` owns transient selected-net connectivity highlighting over graph wires, peer labels, and connected pin anchors. `gui::sketch_net_labels` owns persisted schematic named-net/off-page connector label metadata, badge layout/drawing/hit-testing, typed create-or-reuse placement, inline edit/autocomplete with retarget-or-rename semantics, selected-net rename routing, peer-label navigation, conversion, deletion, cleanup when nets are removed, and net-rename synchronization. `gui::sketch_duplicate` owns selected-component/local-net duplication YAML mutation. `gui::sketch_rename` owns structured component/net rename mutations that rewrite Board IR keys, schematic metadata, generated analog bindings, and generated branch probe expressions before validation, with focused rename regressions in `gui::sketch_rename_tests`. `gui::sketch_spice` owns selected-component SPICE primitive/value mutations for passive and independent-source component evidence, including pin-convention validation and exact branch probe expression rewrites when primitive prefixes change. `gui::sketch_inline_edit` owns direct canvas inline component ID and scalar SPICE value editing while routing through the same validated rename/SPICE mutation helpers. `gui::sketch_palette` owns generic passive and independent-source primitive insertion at the current view, canvas click, drag/drop release with orientation-aware live ghost/snap/pin-side feedback, or context-menu pointer, creating the component, pin nets, SPICE evidence, schematic placement, and non-default schematic orientation through one validated Board IR YAML mutation. `gui::sketch_bundles` owns conservative derived net-bundle grouping, overlay drawing, badge hit-testing, and bundle multi-selection over scalar Board IR nets. `gui::sketch_hierarchy` owns GUI-only schematic hierarchy groups derived from imported KiCad source instance paths and importer namespace prefixes, plus selection, viewport-fit, dim-focus, isolate-view, and off-sheet connector badge overlays over the flattened Board IR graph. `gui::sketch_probes` owns derived analog voltage/current/power probe element targeting, layout, drawing, badge hit-testing, and latest-report assertion status derivation. `gui::sketch_inspector` owns selected-item inspector UI, scalar component/net edits, component/net rename controls, component/net add/remove actions, symbol style edits, pin assignment, visual wire mutations, net-label/off-page connector placement controls, and canvas-driven voltage/current/power probe insertion/removal controls. `gui::sketch_selection_inspector` owns multi-selection summaries, the on-canvas selected-group frame/move handle with snap/free target feedback and quick toolbar with rotate/flip/pin-side actions, and quick actions that reuse existing fit, nudge, align, distribute, orientation, copy, duplicate, and delete paths. `gui::sketch_symbols` owns model/id-inferred common-class symbol selection and egui glyph drawing for sketch nodes. `gui::sketch_actions` owns canvas selection state, Home, Fit All, and Fit Selection viewport commands, group drag/nudge/alignment/distribution, selected-item deletion composition, transient selected-component clipboard state, and selected-component duplicate/copy/paste action composition. `gui::sketch_navigator` owns searchable component, net-bundle, net, wire, and probe navigator rows plus selection and viewport-fit actions for visible sketch targets. `gui::library` owns active model browsing, filtering, selected-component model assignment, and model-backed component insertion/placement at the current view, canvas click, drag/drop release with orientation-aware live ghost/snap/pin-side feedback, or context-menu pointer, including generated default pin nets and non-default schematic orientation. `gui::simulation` owns the Simulation/Scopes stage shell, keeping the runtime oscilloscope plot primary while stage layout, model-run controls, side-dock orchestration, and scope-run preparation stay centralized. `gui::simulation_editors` owns docked scenario/model/source/assertion editors. `gui::simulation_probe_assertions` owns selected-probe assertion table actions plus canvas-triggered assertion add/clear/quick-threshold actions for probe elements. `gui::library_observation_presets` owns Library/Example `Create Checks` orchestration and generated transient scenario/probe insertion. `gui::library_observation_preset_kinds` owns category-specific generated checks for comms outputs, gate drivers, PWM drivers, IMUs, environmental sensors, SPI flash, I2C EEPROM, I2C I/O expanders, secure elements, MCU boot/line-state checks, level shifters, logic buffers, and protection clamps. `gui::analog_ac_presets` owns low-pass, unity-gain, and loop-stability AC/Bode check presets that append ordinary assertion rows. `gui::analog_dc_presets` owns 3.3 V rail, 5 V rail, and 2.5 V midpoint DC operating-point check presets that append ordinary assertion rows. `gui::simulation_sweeps` owns the Run Input Sweeps UI surface and action handlers. `gui::simulation_forms` owns shared Simulation/Scopes form defaults, scenario/net/probe combo widgets, stimulus field loading, and status-color helpers for those docked editors. `gui::waveform` owns Scopes state orchestration, DC operating-point table state, simulation-time scrub/playback, cursor readouts, selected-plus-pinned cursor readout rows, cursor/visible-window min/max/mean/RMS region statistics with snapshot capture, min/max/delta measurements, transient selected-probe trace pinning/comparison overlays, transient per-trace overlay visibility/color styles, split-unit runtime lanes, GUI-only derived waveform channels, promotion of representable derived channels to explicit Board IR analog probes/assertions, and exact probe-value lookup for badge quick assertions. `gui::waveform::waveform_runtime` owns runtime probe matching between loaded waveform artifacts and Sketch selections, graph-hover readout lines, normalized activity values for graph tinting, exact Scope Activity sample rows, bounded Scope Activity sparkline samples, and per-trace edge stepping for schematic-side observation. `gui::waveform::waveform_io` owns streaming, cancel-aware waveform CSV parsing, report/path/request loading, and selected-column waveform requests used by deferred artifact loads. `gui::waveform::waveform_load` owns bounded CSV preflight estimates plus optional large-artifact deferral, selected-column diagnostic merging that marks loaded preview labels, skips duplicate selected-column reloads, preserves full deferred placeholders until full load, and converts unloaded full artifacts back into deferred diagnostics. `gui::waveform::waveform_load_diagnostics` owns filterable/copyable transient waveform-load diagnostics for loaded/deferred/skipped CSV artifacts, including preview-column loaded/unloaded audit metadata, preview-load-state filtering, row-level selected-column load shortcuts, exact preview-column picking, and runtime unload controls for loaded rows. `gui::waveform::waveform_deferred` owns deferred waveform artifact placeholders with header-only probe previews, selector-side filtering, and row/visible/all/matching-column background load actions plus remaining-preview and searchable exact preview-column picker actions with select-visible helpers that mark loaded preview labels, skip duplicate selected-column reloads, and keep full deferred artifacts available after partial column loads. `gui::waveform::waveform_trace_selector` owns waveform and searchable/grouped trace selection, transient saved compare sets, transient trace-style controls, split-unit lane toggling, selected-trace reset behavior, and loaded-artifact unload actions that drop or shift transient trace references. `gui::waveform::waveform_footprint` owns loaded-waveform footprint readouts, compact source memory summaries with CSV/Markdown copy helpers, diagnostics-derived source classification/grouping/filtering, sort/filter projection, visible-row CSV copy/export, memory-budget warnings, and guarded visible-row or largest-first bulk unload preview/confirmation. `gui::waveform::waveform_context` owns pending schematic probe-to-scope focus, runtime trace/event-to-schematic cross-focus selection, selected-trace schematic-context strip actions, and scope probe lookup. `gui::waveform::waveform_plot` owns primary scope plot drawing, draggable/click-set A/B cursor handles, trace overlay selection, transient min/max decimated trace-point caching for large CSV rendering, transient measurement snapshot marker chips with hover and click actions, and shared-axis or per-unit lane axis scaling. `gui::waveform::waveform_export` owns deterministic runtime SVG rendering for the current Scopes plot, including visible traces, split-unit lanes, cursors, trigger markers, snapshot chips, bounded decimated trace polylines, report-size presets, and annotation inclusion toggles. `gui::waveform::waveform_view` owns Scopes plot orchestration, cursor readouts, region statistics display and capture, playback controls, visible time-window and value-window fit/zoom/pan helpers, Back/Forward view-window history, scope plot SVG copy/export actions, and measurement snapshot display. `gui::waveform::waveform_snapshots` owns transient cursor-region, region-stat, trigger-event, and Scope Activity sample and frequency measurement snapshot capture, editable labels/notes, search/source filtering that includes Scope Activity samples and frequency rows, sort/group projection, filtered CSV/Markdown copy/export serialization, interactive plot-marker derivation, Jump restore, schematic Focus, and rendering over loaded waveform artifacts. `gui::waveform::waveform_bundles` owns timestamped report bundle export with the configured or DC-placeholder plot SVG, operating-point CSV/Markdown artifacts, local index page, README manifest, optional artifact integrity detail files, and loaded-waveform footprint source totals. `gui::waveform::waveform_bundle_recent` owns recent-bundle folder/index/integrity-audit opening, path copy actions, integrity problem filtering, guarded refresh, and previewed/confirmed bounded old-bundle cleanup. `gui::waveform::waveform_bundle_integrity` owns report-bundle artifact size/SHA-256 metadata, `artifact_manifest.csv`, missing/changed artifact status checks, expected/current artifact integrity detail rows, and problem-only detail projection. `gui::waveform::waveform_trigger` owns transient selected-trace trigger edge/threshold controls, CSV-derived crossing interpolation, exact event readout rows, and previous/next or row-level trigger jumps, with focused waveform regressions split into `gui::waveform::waveform_tests` for parser/plot/trigger helpers, `gui::waveform::waveform_loading_tests` for waveform loading, deferred-artifact, diagnostics, and footprint behavior, `gui::waveform::waveform_measurement_tests` for cursor, region-stat, and snapshot measurement behavior, `gui::waveform::waveform_scope_activity_tests` for Sketch-side Scope Activity observation/report workflows, `gui::waveform::waveform_bundle_tests` for scope report bundle filesystem and integrity behavior, `gui::waveform::waveform_scope_tests` for app-level Scopes context, lane, probe, and runtime behavior, and `gui::waveform::waveform_scope_compare_tests` for compare pins, trace styles, and saved compare-set state. `gui::analog_overview` owns read-only generated analog scenario audit snapshots for timing/backend, included components, editable source primitives, probes, assertions, SHA-backed model files, node bindings, and readiness diagnostics and editor-navigation actions for missing source/probe/assertion/SHA/binding coverage. `gui::analog_models` owns SHA-backed analog model/include listing, hashing, add, and remove mutations for analog scenarios. `gui::analog_run_setup` owns generated-from-Board analog transient, AC/Bode, and DC operating-point scenario creation, including model-file inference and generated node/pin bindings. `gui::analog` owns expression-probe insertion for promoted waveform math, selected-net voltage-probe insertion, selected-component source/passive/semiconductor current-probe insertion, selected-component source/passive/semiconductor power-probe insertion into existing analog scenarios, probe assertion summaries, probe removal with dependent check cleanup, assertion removal/replacement, assertion removal for a probe, unique assertion naming, and sample/window/timing/duty/count assertion authoring. `gui::analog_generated` owns generated scenario analysis settings, ground/node binding edits, component membership, and pin-binding repair/cleanup. `gui::analog_branches` owns supported source/passive/semiconductor branch current and power expression derivation for component probes. `gui::analog_stimulus` owns structured DC and pulse source-primitive listing and value/timing mutation for components included in generated analog scenarios. `gui::spice` owns file-backed SPICE deck discovery, load/save, and save-and-run actions for analog scenarios. |
| `main` | Own the command-line interface and import/validate/suggest command wiring. |

`gui::sketch_svg` owns deterministic Sketch SVG export for headless visual QA.
The feature-gated `export-sketch-svg` command loads Board IR, runs the same
Sketch graph layout and Fit All bounds used by the GUI, and writes schematic
...
metadata and undo history. The engine follows the graph-drawing primitives used
by ELK, Graphviz, and OGDF: source-seeded ranks, explicit KiCad pin-anchor
ports, power/ground rails, and orthogonal routes. It writes component/net
positions and only the non-default symbol orientations needed for standard
schematic conventions such as vertical ground shunts, then derives display-only
orthogonal wire-route waypoints from the resulting pin anchors and display-only
placed-probe `x`/`y` coordinates in non-overlapping lanes near their pin/wire
targets.
...
untracked problem artifacts, can copy the current detail projection as CSV or
...
overlays: the floating Scope Activity window can be closed and reopened from a
small canvas button, while the `Circuit View` toggle hides or restores the
underlying runtime tint/chip layer without editing Board IR. Scopes `Run` prepares an inspectable waveform path by adding a generated
...
Import, Library, Observations/Scopes, and Reports open as floating overlays or
...
runtime Scope Activity floating window and reopen button, searchable loaded-trace browser,
Cursor A scrub control, per-trace edge stepping, cursor-sampled value/time readouts,
compact frequency/period readouts, bounded per-trace sparklines, live activity-snapshot status, visible-list snapshot capture, row-level and visible-list CSV/Markdown copy, row-level and visible-list report bundle export/index-open, recent-bundle reopen/path-copy shortcuts, source-specific snapshot clearing for sample and frequency rows, source-filtered Open Snapshots routing, compare pin/unpin/clear/named-save/load/delete actions, Open Compare jumps,
and overlay visibility checkbox.
...
| `BoardYamlRepairReport` | `repair_yaml` | Agent-facing patch-and-rerun proof for supported Board IR YAML repairs, currently `INVALID_POWER_DOMAIN` net-kind correction, canonical-id `MODEL_NOT_FOUND` model replacement, `NET_NOT_FOUND` net creation, explicit-metadata `POWER_DOMAIN_NOT_FOUND` power-net creation, `PIN_NOT_DECLARED` stray-pin removal, and metadata-proven `REQUIRED_PIN_FLOATING` pin connection on a copied project. |
...
- JLC/EasyEDA BOM+CPL import adds components and placements plus a JSON
  source/row-hash/component/footprint-key manifest, but does not infer nets,
  pins, or package compatibility from assembly files.
- Reviewed manufacturing metadata CSV import can create or replace reviewed
  ...
  return loss, or RF topology from order text. The
...
configured, and GUI validation progress reports analog run setup preparation,
deck preparation, backend execution, waveform loading, and check/limit

...
compact frequency/period readouts, bounded per-trace sparklines, live activity-snapshot status, visible-list snapshot capture, row-level and visible-list CSV/Markdown copy, row-level report bundle export, source-specific snapshot clearing for sample and frequency rows, source-filtered Open Snapshots routing, compare pin/unpin/clear/named-save/load/delete actions, Open Compare jumps,

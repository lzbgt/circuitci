...
| `gui` | Optional `egui`/`eframe` desktop shell for project loading, staged review, validation, simulation artifact observation, and report viewing. `gui::sketch` owns Board IR graph snapshots/layout, structured scalar component/net edits, conservative component/unreferenced-net add/remove mutations, persisted schematic node positions, visual pin-to-net assignment, validated component pin assignment, and graph hover display. `gui::library` owns active model browsing, filtering, and selected-component model assignment. `gui::simulation` owns the Simulation stage UI, waveform CSV parsing, plotting, cursor readouts, min/max/delta measurements, and runtime probe values for graph hovers. `gui::analog` owns generated-from-Board analog transient scenario creation and sample/min/max assertion authoring. |

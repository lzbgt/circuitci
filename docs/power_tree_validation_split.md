...
limit after adding regulator, load-switch, charger, reset-supervisor,
power-mux, I/O-voltage, and rail-budget checks. The split keeps behavior
unchanged while creating room for more board-validation logic.
...
- `src/validation/power_tree/power_mux.rs` owns `power_mux` validation:
  ...
  diagnostics.
- `src/validation/power_tree/reset_supervisor.rs` owns `reset_supervisor`
  validation: metadata checks, monitored rail nominal-voltage threshold margin,
  and release threshold comparison against monitored-load minimum operating
  voltage.

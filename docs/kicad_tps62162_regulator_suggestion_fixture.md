...
- `UBUCK`, a TPS62162 with `VIN` on `RAIL_12V`, `VOS` on `RAIL_3V3`, `EN`
  tied to `RAIL_12V`, `PG` on `BUCK_PG`, and `GND` on ground.
...
evidence is available to agents from schematic import, not buck inductor
selection, ripple, control-loop stability, thermal behavior, EMI, or layout
sign-off.

...
- When `board.manufacturing.controlled_impedance.coupons[]` contains reviewed
  coupon measurements with explicit targets and tolerances that map to exactly
  one matching reviewed board controlled-impedance net or differential-pair
  target, it emits runnable `CONTROLLED_IMPEDANCE_COUPON_VALID` templates. The
...
- When reviewed
  ...
  source-backed solver metadata, solver artifact URI/digest evidence, complete
  ...
  evidence, material-library evidence, material-corner evidence, any declared
  reviewed fabricator stackup signoff evidence, any declared reviewed solver
  ...
  input-deck, roughness, etch-compensation, material-library, material-corner,
  stackup-signoff, sweep, and qualification evidence.

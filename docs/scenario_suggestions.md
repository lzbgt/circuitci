...
- It emits non-runnable `USB_CONNECTOR_EDGE_PROXIMITY_VALID` templates when the
  ...
  edges from sampled `gr_rect`, `gr_circle`, or `gr_arc` edges and external
  contours from cutouts. Segments marked `boundary_role: cutout` are not
  selected as USB connector entry edges. When imported footprint drawing evidence exists,

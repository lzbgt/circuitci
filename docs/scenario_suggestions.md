...
- When reviewed
  ...
  board-target metadata, stackup layer references, and imported route evidence,
  it emits runnable `CONTROLLED_IMPEDANCE_SOLVER_RESULT_VALID` templates. The
  suggestion does not run a solver or fetch artifacts; it only surfaces
  explicit solver-result evidence.

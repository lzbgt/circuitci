...
report. Matching-column and exact preview-column picker loads append selected traces, mark loaded preview labels, skip already loaded columns, and preserve the full
...
with header-only probe previews, selector-side filtering, and row/visible/all, matching-column, or exact preview-column picker background load actions that mark loaded preview labels, skip duplicate selected-column reloads, and keep full deferred artifacts available after partial column loads, transient saved compare sets, transient

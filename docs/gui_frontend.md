...
`src/gui/waveform.rs` owns streaming, cancel-aware waveform CSV parsing, transient waveform-load diagnostics, Scopes state orchestration,
...
progress/cancel checks and record loaded/skipped file diagnostics before the
GUI applies the completed report, keeping large CSV files out of the UI thread
while making missing traces explainable in Scopes.

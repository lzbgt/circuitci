...
measurement tools, selected-plus-pinned cursor readout table, cursor/visible-window region statistics, actionable transient cursor-region and trigger-event measurement snapshots with plot markers, GUI-only
...
- Simulation/Scopes: presents a runtime-first oscilloscope workspace linked
  ...
  selected-plus-pinned A/B cursor readouts, cursor/visible-window region statistics with min/max/mean/RMS rows, and transient measurement snapshots from cursor regions or trigger events with interactive plot marker chips plus row-level Jump and schematic Focus actions.

...
measurement tools, selected-plus-pinned cursor readout table, actionable transient cursor-region and trigger-event measurement snapshots with plot markers, GUI-only
...
trace overlay selection, transient measurement snapshot marker chips, and shared-axis or per-unit lane axis scaling.
...
- Simulation/Scopes: presents a runtime-first oscilloscope workspace linked
  ...
  selected-plus-pinned A/B cursor readouts, and transient measurement snapshots from cursor regions or trigger events with plot marker chips plus row-level Jump and schematic Focus actions.
...
Visible snapshot marker chips are derived from the same transient rows and
draw only for currently visible selected/pinned traces.

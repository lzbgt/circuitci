...
- Project: choose a Board IR project and output directory by typing paths or
  ...
  fixture, and comparator threshold fixture; all direct-open projects include
  display-only routed schematic waypoints. Opening one lands directly in Sketch
  with a deferred Fit All, so the first view is the readable connected network.
  When a scope-ready fixture is active, the Project overlay also shows a

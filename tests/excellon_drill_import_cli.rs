...
fn import_excellon_drill_appends_schema_valid_drill_evidence() {
    ...
    let slots = imported["board"]["layout"]["slots"].as_array().unwrap();
    ...
    assert_eq!(slots[0]["plating"], "plated");
    assert_eq!(slots[0]["layer"], "PTH_Through");
    assert_eq!(slots[0]["tool"], "T08");
    assert_eq!(slots[0]["source_slot_index"], 0);
    assert_eq!(slots[0]["width_mm"], 3.2004);
    assert_eq!(slots[0]["start"]["x_mm"], 6.72504);
    assert_eq!(slots[0]["end"]["x_mm"], 5.82504);
...
#[test]
fn import_excellon_drill_reports_routed_slots() {
    std::fs::create_dir_all("out").unwrap();
    let dir = tempfile::tempdir_in("out").unwrap();
    let drill = dir.path().join("slots.drl");
    let output = dir.path().join("with_slots.project.yaml");
    std::fs::write(
        &drill,
    ...
    assert!(command_output.status.success());
    let stdout = String::from_utf8_lossy(&command_output.stdout);
    assert!(stdout.contains("0 hits, 1 routed slots"));

...
#[test]
fn generated_ch340n_usb_uart_observation_uses_datasheet_backed_model_pack() {
    let report = run_validation("examples/good_wch_ch340n_usb_uart_observation/project.yaml");
    if binary_available("ngspice") {
        assert_eq!(report["result"], "pass");
        assert_eq!(report["summary"]["critical"], 0);
        assert!(report["failures"].as_array().unwrap().is_empty());
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/wch/ch340n_usb_uart.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_bme280_i2c_observation_uses_datasheet_backed_model_pack() {
    let report = run_validation("examples/good_bosch_bme280_i2c_observation/project.yaml");
    if binary_available("ngspice") {
        assert_eq!(report["result"], "pass");
        assert_eq!(report["summary"]["critical"], 0);
        assert!(report["failures"].as_array().unwrap().is_empty());
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/bosch/bme280_i2c_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_sht31_i2c_observation_uses_datasheet_backed_model_pack() {
    let report = run_validation("examples/good_sensirion_sht31_i2c_observation/project.yaml");
    if binary_available("ngspice") {
        assert_eq!(report["result"], "pass");
        assert_eq!(report["summary"]["critical"], 0);
        assert!(report["failures"].as_array().unwrap().is_empty());
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/sensirion/sht31_i2c_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_w25q64jv_spi_flash_observation_uses_datasheet_backed_model_pack() {
    let report =
        run_validation("examples/good_winbond_w25q64jv_spi_flash_observation/project.yaml");
    if binary_available("ngspice") {
        assert_eq!(report["result"], "pass");
        assert_eq!(report["summary"]["critical"], 0);
        assert!(report["failures"].as_array().unwrap().is_empty());
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/winbond/w25q64jv_spi_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_mcp23017_i2c_gpio_expander_observation_uses_datasheet_backed_model_pack() {
    ...
    if binary_available("ngspice") {
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/microchip/mcp23017_i2c_gpio_expander_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_atecc608a_i2c_secure_element_observation_uses_datasheet_backed_model_pack() {
    let report = run_validation(
    ...
    if binary_available("ngspice") {
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/microchip/atecc608a_i2c_secure_element_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_nau7802_bridge_adc_observation_uses_datasheet_backed_model_pack() {
    let report =
        run_validation("examples/good_nuvoton_nau7802_bridge_adc_observation/project.yaml");
    if binary_available("ngspice") {
        assert_eq!(report["result"], "pass");
        assert_eq!(report["summary"]["critical"], 0);
        assert!(report["failures"].as_array().unwrap().is_empty());
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/nuvoton/nau7802_bridge_adc_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
...
#[test]
fn generated_aht20_i2c_observation_uses_resolved_datasheet_backed_model_pack() {
    let report = run_validation("examples/good_aosong_aht20_i2c_observation/project.yaml");
    if binary_available("ngspice") {
        assert_eq!(report["result"], "pass");
        assert_eq!(report["summary"]["critical"], 0);
        assert!(report["failures"].as_array().unwrap().is_empty());
    ...
    if binary_available("ngspice") {
        ...
        assert!(artifacts.iter().any(|artifact| {
            artifact
                .as_str()
                .unwrap()
                .ends_with("models/spice/nordic/nrf52840_board_observation.lib")
        }));
        assert!(
            artifacts
                .iter()
